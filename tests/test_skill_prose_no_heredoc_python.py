...
def test_vision_evaluator_gate_has_no_python_heredoc():
    """Evaluator gate phase must invoke spec_evaluator via spectre wrapper CLI."""
    text = _VISION_SKILL.read_text()
    body = _section_text(
        text,
        r"^### Phase: Evaluator gate — spec evaluation.*$",
        r"^### Phase:.*$",
    )
    assert _heredoc_count(body) == 0, (
        "Evaluator gate phase must use `spectre spec_evaluator evaluate` "
        "CLI — heredoc-python was removed in Phase 2B (issue #13)."
    )
    assert "spectre spec_evaluator evaluate" in body, (
        "Evaluator gate phase must invoke `spectre spec_evaluator evaluate`."
    )


def test_vision_lock_phase_references_eval_bundle():
    """Lock phase must reference state/.eval-bundle.json for resource nodes."""
    text = _VISION_SKILL.read_text()
    body = _section_text(
        text,
        r"^### Phase: Lock.*$",
    ...
    assert _heredoc_count(body) == 0, (
        "Lock phase must not use heredoc-python — removed in Phase 2B (issue #13)."
    )
    assert "state/.eval-bundle.json" in body, (
        "Lock phase must reference state/.eval-bundle.json."
    )


def test_vision_lock_sidecar_write_uses_spectre_cli():
    """Lock phase sidecar-write block must use the spectre eval_metadata CLI."""
    text = _VISION_SKILL.read_text()
    body = _section_text(
        text,
    ...
    assert _heredoc_count(body) == 0, (
        "Sidecar-write block must use `spectre eval_metadata write-sidecar` "
        "CLI — heredoc-python was removed in Phase 2B (issue #13)."
    )
    assert "spectre eval_metadata write-sidecar" in body, (
        "Sidecar-write block must invoke the write-sidecar CLI via spectre wrapper."
    )


# ── Phase 2C replacements: implement Tier classifier, Verify ──────────────────


def test_implement_tier_classifier_phase_has_no_python_heredoc():
    """Tier classifier phase must invoke `spectre tier evaluate-action` via CLI.

    Scoped to the classifier block (from the Tier classifier heading to the
    **Record observation sub-section) — other sub-sections within the phase
    are covered by the global Phase 2D zero-ceiling guard.
    """
    text = _IMPLEMENT_SKILL.read_text()
    body = _section_text(
        text,
        r"^### Phase: Tier classifier.*$",
        r"^\*\*Record observation BEFORE accepting input:\*\*.*$",
    )
    assert _heredoc_count(body) == 0, (
        "Tier classifier phase must use `spectre tier evaluate-action` "
        "CLI — heredoc-python was removed in Phase 2C/2D (issue #13)."
    )
    assert "spectre tier evaluate-action" in body, (
        "Tier classifier phase must invoke `spectre tier evaluate-action`."
    )


def test_implement_verify_phase_has_no_python_heredoc():
    """Verify phase (State Auditor) must invoke `spectre auditor audit-and-clear` via CLI."""
    text = _IMPLEMENT_SKILL.read_text()
    body = _section_text(
        text,
        r"^### Phase: Verify.*$",
        r"^### Phase: Branch on verification.*$",
    )
    assert _heredoc_count(body) == 0, (
        "Verify phase must use `spectre auditor audit-and-clear` "
        "CLI — heredoc-python was removed in Phase 2C/2D (issue #13)."
    )
    assert "spectre auditor audit-and-clear" in body, (
        "Verify phase must invoke `spectre auditor audit-and-clear`."
    )
